      ...
      <div className="left-0 w-full z-50 bg-white rounded-t-xl shadow-lg flex flex-col">
        <div className="bg-[#7b5449] text-white">
          <ul className="cursor-pointer m-0 p-0 flex flex-row justify-around w-full">
            <li onClick={() => setActiveStep(1)} className="p-3">
              <i className="bi bi-image text-2xl"></i>
            </li>
            <li onClick={() => setActiveStep(2)} className="p-3">
              <i className="bi bi-brush text-2xl"></i>
            </li>
            <li onClick={() => setActiveStep(3)} className="p-3">
              <i className="bi bi-person-square text-2xl"></i>
            </li>
            <li onClick={() => setActiveStep(4)} className="p-3">
              <i className="bi bi-palette text-2xl"></i>
            </li>
            <li onClick={() => setActiveStep(5)} className="p-3">
              <i className="bi bi-sliders text-2xl"></i>
            </li>
            <li onClick={() => setActiveStep(6)} className="p-3">
              <i className="bi bi-stars text-2xl"></i>
            </li>
            <li onClick={() => setActiveStep(7)} className="p-3">
        ...
        <div className="bg-gray-100 py-1 px-3 overflow-y-auto h-[20vh]">
          {activeStep === 1 && (
            <div className="flex flex-col gap-3 py-12 justify-center">
              ...
              <label
                htmlFor="fileUpload"
                className="cursor-pointer border-2 border-dashed border-[#7b5449] text-stone-500 font-base text-center py-3 rounded-xl shadow-md hover:bg-[#694237] transition text-lg"
              >
                이미지 등록하기
              </label>
            </div>
          ...
          {activeStep === 2 && (
            <p className="font-base text-sm text-center">빠른 제작</p>
          )}

          {activeStep === 3 && (
            <div>
              <p className="font-base text-sm text-center">디자인템플릿</p>
          ...
          {activeStep === 4 && (
            <div>
              <p className="font-base text-center text-sm">배경설정</p>
              <div className="flex flex-wrap gap-3 mt-2">
                {backgrounds.map((bg) => (
                  <div
                    key={bg.id}
                    className="w-[60px] h-[60px] rounded-lg cursor-pointer hover:scale-105 transition"
                    style={{ backgroundColor: bg.color }}
                    onClick={() => setBackground(bg.color)}
                  >
                    <p className="text-[12px] text-center text-gray-800">
                      {bg.name}
                    </p>
                  </div>
                ))}
          ...
          {activeStep === 5 && (
            <div>
              <p className="font-base text-center text-sm mb-4">사진보정</p>
          ...
          {activeStep === 6 && (
            <div>
              <p className="font-base text-center text-sm">필터효과</p>
              <div className="flex flex-wrap gap-3 mt-2">
                {presets.map((preset) => (
                  <div
                    key={preset.id}
          ...
          {activeStep === 7 && (
            <div>
              <p className="font-base text-center text-sm">저장하기</p>
              <p>사진 저장하기</p>
              <p>사진 공유하기</p>
            </div>
          )}
